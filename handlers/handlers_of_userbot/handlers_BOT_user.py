...
from Database.database import db_get_set_of_parse_channel
...
all_parse_channel_set = db_get_set_of_parse_channel()

def join_chat(id_chat):
    pass
# ____________________________________________________________
# handlers for telegram client
@telethon.events.register(telethon.events.NewMessage())
async def event_handler_ms_from(event):
    print(all_parse_channel_set)
    print(str(event.message.sender_id))
    if str(event.message.sender_id) not in all_parse_channel_set or event.message.sender_id == 5864612640:
